...
def refresh_cot_data(cot_log):
    ...
    for inst, cfg in COT_INSTRUMENTS.items():
        ...
        if df.empty:
            continue
        for col in ['lev_money_positions_long_all', 'lev_money_positions_short_all', 'open_interest_all']:
            df[col] = pd.to_numeric(df[col], errors='coerce')
        df['lev_net_pct_oi'] = (df['lev_money_positions_long_all'] - df['lev_money_positions_short_all']) / df['open_interest_all']
